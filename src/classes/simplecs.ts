...
export class SimplecsCalculator {
  ...
  public calculate = () => {
    ...
    return this.results() // Отображаем результаты в понятном виде
  }
  ...
  private step2() {
    ...
    // нужно ли еще разок пройти второй шаг ?
    if (
      this.minelm(this.matrix[this.matrix.length - 1], false, true) < 0 &&
      this.iteration < 10
    )
  ...
  private results() {
    ...
    for (let i = 0; i < this.horisont_x.length - 1; i++) {
      if (this.horisont_x[i] < InputStoreService.getMaxX())
        nulls += 'X' + (this.horisont_x[i] + 1) + '='
    }
    nulls += '0 <br /><br />'
    let vars = ''
    ...
    for (let i = 0; i < this.vertical_x.length; i++) {
      if (this.vertical_x[i] < InputStoreService.getMaxX())
        vars +=
          ...
          (this.vertical_x[i] + 1) +
          '=' +
          this.matrix[i][InputStoreService.getMaxX()] +
          '<br />'
    }
    var main_result = ''
    ...
    main_result =
      'min F =' +
      this.matrix[this.matrix.length - 1][InputStoreService.getMaxX()] * -1
  ...
  private minelm = (
    v: number[],
    dispnum: boolean = false,
    not_last: boolean = false
  ) => {
    var m = v[0]
    var num = 0
    var len = 0
  ...
  private print_table(arr: number[][], row?: number, col?: number) {
    ...
    if (result) {
      result.innerHTML = '<table>' + html_head + html_table + '</table>'
    }
    //Выделяем колонку, если указана
    if (col !== undefined)
      document.querySelectorAll('td').forEach(item => {
        // console.log(item.cellIndex)
        if (item.cellIndex === col) {
