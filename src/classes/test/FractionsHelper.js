...
export function Fraction(text = '0') {
  if (
    text != '' &&
    text.match(new RegExp(fractionTemplate)) == null &&
    text.match(new RegExp(realTemplate)) == null
  )
    throw 'Некорректная дробь!'
...
Fraction.prototype.print = function(view = 1, digits = 5) {
  ...
  if (view == 2) {
    ...
    return (
      (this.n.isNegative() ? '-' : '') + int + (real == '0' ? '' : '.' + real)
    )
  }
...
Fraction.prototype.add = function(b) {
  ...
  result.n = this.n
    .multiply(nok)
    .divide(this.m)
    .plus(b.n.multiply(nok).divide(b.m))
  result.m = nok
  result.reduce()
  return result
}
Fraction.prototype.sub = function(b) {
  ...
  result.n = this.n
    .multiply(nok)
    .divide(this.m)
    .minus(b.n.multiply(nok).divide(b.m))
  result.m = nok
  result.reduce()
  return result
}
